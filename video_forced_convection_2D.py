...
run_time = 5000 # total run time in seconds
...
exp_name_short = 'forced-convection_with_new_forcing'
...
energy_plots_dir = os.path.join('energy_plots', experiment_name)
...
if rank == 0:
    ...
    if not os.path.exists(energy_plots_dir): os.makedirs(energy_plots_dir)
...
def diffusive_forcing(solver, state, dstatedt):
    ...
    # New Forcing code
    s_profile = np.copy(s)
    s_forcing = -(s-s_profile)*(solver.zs > 6000)/60
...
if run_model:
    ...
    for i, tend in enumerate(tends):
        ...
        if rank == 0:
            total_energy.append(solver.energy())
            potential_energy.append(solver.energy_sep()[0])
            kinetic_energy.append(solver.energy_sep()[1])
            internal_energy.append(solver.energy_sep()[2])
            min_temp.append(solver.T.min())
            max_temp.append(solver.T.max())
...
x = len(kinetic_energy)
# print(x)
X = np.linspace(0, 50*(x-1), num = x)
# print(X) 
plt.semilogy(X, total_energy, color='r', label='Total Energy')
plt.semilogy(X, internal_energy, color='g', label='Internal Energy')
plt.semilogy(X, potential_energy, color='b', label='Potential Energy')
plt.semilogy(X, kinetic_energy, color='y', label='Kinetic Energy')
plt.legend()
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/All_Energy.png'.format(exp_name_short))
plt.clf()

plt.plot(X, total_energy, color='r', label='Total Energy')
plt.legend()
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Total_Energy.png'.format(exp_name_short))
plt.clf()

plt.plot(X, internal_energy, color='g', label='Internal Energy')
plt.legend()
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Internal_Energy.png'.format(exp_name_short))
plt.clf()

plt.plot(X, potential_energy, color='b', label='Potential Energy')
plt.legend()
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Potential_Energy.png'.format(exp_name_short))
plt.clf()

plt.plot(X, kinetic_energy, color='y', label='Kinetic Energy')
plt.legend()
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Kinetic_Energy.png'.format(exp_name_short))
plt.clf()

plt.plot(X, min_temp, color='g', label='Min Temp (K)')
plt.plot(X, max_temp, color='r', label='Max Temp(K)')
plt.legend()
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Max_Min_temp.png'.format(exp_name_short))
plt.clf()